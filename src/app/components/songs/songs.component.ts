...
import { SongDto } from '../../models/admin.model';
    ...
    <app-layout>
      <div class="page">
        <div class="header">
          <h1>Songs</h1>
          <div class="search">
            <input type="text" [(ngModel)]="searchQuery" placeholder="Search songs..." (keyup.enter)="search()" />
        ...
        <div class="table-container">
          <table>
            <thead>
              <tr><th>Title</th><th>Artist</th><th>Genre</th><th>Plays</th><th>Likes</th><th>Actions</th></tr>
            </thead>
            <tbody>
              @for (song of songs(); track song.id) {
                <tr>
                  ...
                  <td>{{ song.totalLikes }}</td>
                  <td><button class="btn-danger" (click)="deleteSong(song)">Delete</button></td>
                </tr>
              }
            </tbody>
          </table>
        ...
        <div class="pagination">
          ...
          <span>Page {{ page() }}</span>
          <button (click)="nextPage()" [disabled]="songs().length < pageSize">Next</button>
        </div>
      </div>
    </app-layout>
  `,
...
export class SongsComponent implements OnInit {
    ...
    ngOnInit(): void { this.loadSongs(); }

    loadSongs(): void {
        this.adminService.getSongs(this.page(), this.pageSize, this.searchQuery || undefined).subscribe({
            next: (songs) => this.songs.set(songs)
        });
    }

    search(): void { this.page.set(1); this.loadSongs(); }
    prevPage(): void { if (this.page() > 1) { this.page.update(p => p - 1); this.loadSongs(); } }
    nextPage(): void { this.page.update(p => p + 1); this.loadSongs(); }

    deleteSong(song: SongDto): void {
        if (confirm(`Delete song "${song.title}"?`)) {
            this.adminService.deleteSong(song.id).subscribe({ next: () => this.loadSongs() });
        }
    }
}
